...
void main() => runApp(
      MaterialApp(
        theme: ThemeData.light().copyWith(scaffoldBackgroundColor: primaryAccent),
...
class DialItemRenderBox extends RenderBox {
  ...
  void _onDragUpdate(DragUpdateDetails details) {
    ...
    final diffInAngle = toCartersian(previousOffset, radius) - toCartersian(_currentDragOffset, radius);
...
class DialRenderBox extends RenderBox with ContainerRenderObjectMixin<DialItemRenderBox, DialParentData> {
  ...
  void performLayout() {
    ...
    size = Size(firstChildRadius * 2, firstRadius + previousRadius + (DialItemRenderBox.indicatorHeight * 5 / 2));
  }
